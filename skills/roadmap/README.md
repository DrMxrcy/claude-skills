...
| `/roadmap:tidy [--externalize]` · `/roadmap-tidy` | Groom the Idea Incubator / free-form region — prose walls into linked `.roadmap/notes/` files, one bullet per idea, ideas curated against the current roadmap (merge/drop/promote, with approval); `--externalize` moves the incubator into `.roadmap/IDEAS.md` so `ROADMAP.md` stays a pure dashboard |
...
- **Two files render automatically on every `sync`**, both grouped into
  **✨ New / 🐛 Fixed / ⚡ Improved**:
  - **`CHANGELOG.md`** (public) — only `audience: public` items, rendered from each item's
    **`note` only, never the raw title**. A shipped public item with no note is covered by the
    roll-up line (and warned) so a planning title can never leak — and a shipped version never
    vanishes from the file. Versions that also shipped internal-only work get one
    **"behind-the-scenes" roll-up line** instead of listing it. This is the file you paste
    into the **App Store "What's New"** or a website changelog.
...
└── notes/*.md      # long-form idea write-ups, linked from the Idea Incubator
...
- **`ROADMAP.md`** is regenerated by `sync` between the `<!-- roadmap:auto:start -->` /
  `<!-- roadmap:auto:end -->` markers. Fully shipped versions below the current one collapse
  to a one-line summary (full history stays in `CHANGELOG.internal.md`; opt out with
  `settings.collapseShipped: false`). Everything outside the markers (your **Idea
  Incubator** / backlog) is free-form and never rewritten — `idea` and `remove` only append
  single bullets there, and `status` warns if the file outgrows its bounds. The model never
  hand-maintains the dashboard, so it can't drift. Full schema:
